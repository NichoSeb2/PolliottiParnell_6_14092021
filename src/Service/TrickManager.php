...
class TrickManager {
	...
	public function __construct(private MediaUploader $mediaUploader, private TranslatorInterface $translator) {}

	public function processData(FormInterface $form, string $mode, Trick $trick, User $user): void {
		$coverImageFile = $form->get("coverImage")->get("file")->getData();
		...
		switch ($mode) {
			case self::CREATE_MODE:
				$trick->getCoverImage()
                    ...
                    ->setAlt($form->get("coverImage")->get("alt")->getData())
                ;
			...
			case self::EDIT_MODE:
				if (!is_null($coverImageFile)) {
					if ($this->mediaUploader->isValidImage($coverImageFile, Media::ACCEPT_MIME_TYPE)) {
						...
						$form->get("coverImage")->addError(new FormError($this->translator->trans("form.trick.cover-image.wrong-mime-type", [], "validators")));
					}
				}
		...
		foreach ($form->get("medias") as $index => $mediaData) {
			/** @var Media $media */
